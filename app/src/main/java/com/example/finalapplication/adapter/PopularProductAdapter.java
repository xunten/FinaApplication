...
public class PopularProductAdapter extends RecyclerView.Adapter<PopularProductAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_product_popular, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        Glide.with(context).load(popularProductModelList.get(position).getImg_url()).into(holder.item_pro_image);
//        holder.item_pro_description.setText(popularProductModelList.get(position).getDescription());
        holder.item_pro_name.setText(popularProductModelList.get(position).getName());
        DecimalFormat decimalFormat = new DecimalFormat("###,###,###");
        holder.item_pro_price.setText(decimalFormat.format(popularProductModelList.get(position).getPrice()) + " VND");
